    ...
    <>
      <div className='flex items-center justify-between flex-wrap gap-[20px] w-full'>
        {cardData.map((item, index) => {
          return (
            <>
              <div className='h-full lg:h-[386px] w-full  xl:max-w-[470px] 2xl:max-w-[425px] lg:p-[45px]   p-[35px] bg-tc-light dark:bg-tc-tertiary rounded-[20px] font-montserrat  shadow-[0px_0px_10px_2.616px_rgba(0,0,0,0.15)]'>
                <div className='flex items-center justify-between mb-[63px]  w-full'>
                  <h2 className='text-tc-dark dark:text-tc-light text-[24px] font-semibold leading-[26px] '>
                    {item.title}
                  </h2>
                  <div className='py-[10px] px-[20px]  h-[40px] bg-tc-primary  rounded-[30px]'>
                    {item.svg}
                  </div>
                </div>
                {item.points.map((pItem, index) => (
                  ...
                  >
                    <input
                      type='checkbox'
                      className={`w-[14px] h-[14px] border-[1px] rounded-[2px] border-tc-primary mr-[20px] ${
                        checkedIds.includes(pItem.id)
                          ? 'bg-tc-primary'
                          : 'bg-red-500'
                      }`}
                    />
                    <h2 className='text-tc-tertiary dark:text-[#8B8B8B] lg:text-[16px] text-[20px] font-normal leading-[26px]'>
                      {pItem.name}
                    </h2>
                  </div>
                ))}
